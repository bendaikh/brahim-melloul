...
<div class="max-w-2xl">
    ...
    <div class="bg-dark-800 rounded-2xl border border-white/10 p-6">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <div>
                <h2 class="text-lg font-semibold text-white">Modifier la marque</h2>
                <p class="text-sm text-white/50">Mettez à jour les informations de la marque</p>
            </div>
        </div>

        <form action="{{ route('admin.parametres.marques.update', $brand) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    ...
                    placeholder="Description de la marque">{{ old('description', $brand->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <label for="logo" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-white/20 rounded-xl cursor-pointer hover:border-brand-500/50 transition bg-white/5">
                    <div id="image-preview" class="{{ $brand->logo ? '' : 'hidden' }}">
                        <img id="preview-img" src="{{ $brand->logo ? asset($brand->logo) : '' }}" alt="Preview" class="max-h-40 object-contain">
                    </div>
                    <div id="upload-placeholder" class="{{ $brand->logo ? 'hidden' : 'flex flex-col items-center' }}">
                        <svg class="w-12 h-12 text-white/30 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                        </svg>
                        <p class="text-white/50 text-sm">Cliquez pour modifier le logo</p>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-white/10">
                ...
                <button type="submit" class="flex items-center space-x-2 bg-brand-500 hover:bg-brand-600 text-white px-6 py-2.5 rounded-xl font-semibold transition shadow-lg shadow-brand-500/25">
                    ...
                    <span>Mettre à jour la marque</span>
                </button>
            </div>
        </form>
    </div>
