...
@section('title', 'Modifier le représentant')
@section('page-title', 'Modifier le représentant')
@section('page-description', 'Modifier les informations du représentant')
...
<div class="max-w-2xl">
    ...
    <div class="bg-dark-800 rounded-2xl border border-white/10 p-6">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <div>
                <h2 class="text-lg font-semibold">Modifier: {{ $representant->name }}</h2>
                <p class="text-sm text-white/50">Modifiez les informations ci-dessous</p>
            </div>
        </div>

        <form action="{{ route('admin.representants.update', $representant) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-white/70 mb-2">Nom complet <span class="text-red-400">*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $representant->name) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $representant->email) }}"
                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:border-brand-500 focus:outline-none transition"
                        placeholder="email@exemple.com">
                </div>
                <div>
                    <label for="phone" class="block text-sm font-medium text-white/70 mb-2">Téléphone</label>
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $representant->phone) }}"
                        class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:border-brand-500 focus:outline-none transition"
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3"
                    ...
                    placeholder="Adresse complète du représentant">{{ old('address', $representant->address) }}</textarea>
            </div>

            <div>
                <label for="commission_rate" class="block text-sm font-medium text-white/70 mb-2">Taux de commission (%)</label>
                <div class="relative">
                    <input type="number" step="0.01" min="0" max="100" name="commission_rate" id="commission_rate" value="{{ old('commission_rate', $representant->commission_rate) }}"
            ...
            <div class="flex items-center space-x-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $representant->is_active ? 'checked' : '' }}
