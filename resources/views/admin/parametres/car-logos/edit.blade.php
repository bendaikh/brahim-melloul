...
<div class="max-w-2xl">
    ...
    <div class="bg-dark-800 rounded-2xl border border-white/10 p-6">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <div>
                <h2 class="text-lg font-semibold">Modifier: {{ $carLogo->name }}</h2>
                <p class="text-sm text-white/50">Modifiez les informations ci-dessous</p>
            </div>
        </div>

        <form action="{{ route('admin.parametres.car-logos.update', $carLogo) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-white/70 mb-2">Nom de la marque <span class="text-red-400">*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $carLogo->name) }}" required
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <label for="image" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-white/20 rounded-xl cursor-pointer hover:border-brand-500/50 transition bg-white/5">
                        <div id="image-preview" class="{{ $carLogo->image ? '' : 'hidden' }}">
                            <img id="preview-img" src="{{ $carLogo->image ? asset($carLogo->image) : '' }}" alt="Preview" class="max-h-40 object-contain">
                        </div>
                        <div id="upload-placeholder" class="{{ $carLogo->image ? 'hidden' : '' }} flex flex-col items-center">
                ...
                @if($carLogo->image)
                    <p class="text-xs text-white/40 mt-2">Image actuelle: {{ basename($carLogo->image) }}</p>
                @endif
            </div>

            <div class="flex items-center space-x-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $carLogo->is_active ? 'checked' : '' }}
